...
public class CrvenKrst {
    ...
    public static void main(String[] args) {
        ...
        int n = sc.nextInt();
        OBHT<String, Integer> mapa = new OBHT<String, Integer>(2 * n + 1);
        for (int i = 0; i < n; i++) {
            String name = sc.next();
            String bloodType = sc.next().replaceAll("[1-2]", "");
            int koficka = mapa.search(bloodType);
            if (koficka == -1) {
                mapa.insert(bloodType, 1);
            } else {
                mapa.insert(bloodType, mapa.getBucket(koficka).value + 1);
            }
        }
