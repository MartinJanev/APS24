...
public class AdjacencyMatrixGraphUpdated<T> {

    class Edge implements Comparable<Edge> {
        private int fromVertex, toVertex, weight;
    ...
    public AdjacencyMatrixGraphUpdated(int numVertices) {
    ...
    public List<Edge> kruskal() {
        ...
        Set<Integer>[] sets = new HashSet[numVertices];
        for (int i = 0; i < numVertices; i++) {
            sets[i] = new HashSet<>();
            sets[i].add(i);
        }
        ...
        for (Edge e : allEdges) {
            int fromSetIndex = findSetIndex(sets, e.getFrom());
            int toSetIndex = findSetIndex(sets, e.getTo());

            if (fromSetIndex != toSetIndex) {
                mstEdges.add(e);
                unionSets(sets, e.getFrom(), e.getTo());
            }
        }
        return mstEdges;
    }
    ...
    private void unionSets(Set<Integer>[] sets, int fromSetIndex, int toSetIndex) {
        sets[fromSetIndex].addAll(sets[toSetIndex]);
        sets[toSetIndex].clear();
    }


    private int findSetIndex(Set<Integer>[] sets, int vertex) {
