...
public class Patista {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < M; i++) {
            ...
            int index1 = graph.indexOfVertex(city1);
            int index2 = graph.indexOfVertex(city2);

            graph.addEdge(index1, index2, price);
        }

        List<Edge> MST_edges = graph.kruskal();

        //Pravi mapiranje na site rebra vo svoite tezini i gi sumira
        System.out.println(MST_edges.stream().mapToInt(Edge::getWeight).sum());
    ...
    static class AdjacencyMatrixGraph<T> {
        ...
        public int indexOfVertex(T vertex) {
            for (int i = 0; i < numVertices; i++) {
