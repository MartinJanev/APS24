    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (input[0].equals("insert")) {
                ...
                System.out.println(smaller(drvo.root, jazol));
            }
        }
    }

    static int smaller(BNode<Integer> node, int target) {
        if (node == null) return 0;
        if (node.info >= target) return smaller(node.left, target);
        return smaller(node.right, target) + smaller(node.right, target) + 1;
    }
