    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (Objects.equals(tokens[0], "root")) {
                ...
                System.out.println(coundInsideNodesR(node));
            }
        }
    }

    public static int coundInsideNodesR(BNode<String> node) {
        if (node == null) return 0; // ako ima weird (also stop uslov)
        if ((node.left == null) && (node.right == null)) return 0; //(ako nema voopsto deca) -> list

        return coundInsideNodesR(node.left) + coundInsideNodesR(node.right) + 1; // samiot i decata dali se vnatresni
    }
