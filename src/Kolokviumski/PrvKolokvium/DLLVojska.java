...
//На командантот не му се допаѓа како се наредени војниците и решава да одбере два под-интервали од војници и да
// им ги замени местата, односно војниците што се наоѓаат во едниот под-интервал ќе ги смести во другиот, и обратно.
//Влез: Во првиот ред даден е бројот на војници. Во вториот ред дадени се ID-то на секој од војниците. Во третиот \
//ред дадени се два броеви, ID на првиот војник и ID на последниот војник од првиот интервал.
...
// Забелешка 2: Обратете посебно внимание кога интервалите се еден до друг и кога некој од интервалите почнува
...
public class DLLVojska {
    ...
    private static DLL<Integer> vojska(DLL<Integer> lista, int a, int b, int c, int d) {
        ...
        DLLNode<Integer> prvPrva = null;
        DLLNode<Integer> posPrva = null;
        DLLNode<Integer> prvVtora = null;
        DLLNode<Integer> posVtora = null;
        while (node != null) {
            if (node.element == a) {
                prvPrva = node;
                ...
                prvVtora = node;
            } else if (node.element == d) {
                posVtora = node;
            }
            node = node.succ;
        }
        DLLNode<Integer> posleden = posVtora.succ;
        DLLNode<Integer> prv = prvPrva.pred;
        if (a == b) {
            DLLNode temp = prvPrva.succ;
            posVtora.succ = prvPrva.succ;
            prvPrva.pred = prvVtora.pred;
            prvPrva.succ = posleden;
            prvVtora.pred = prv;
            temp.pred = temp.succ;
            temp.succ = prvPrva;

        } else if (c == d) {
            DLLNode temp = prvVtora.succ;
            posVtora.succ = prvPrva.succ;
            prvPrva.pred = prvVtora.pred;
            prvPrva.succ = posleden;
            prvVtora.pred = prv;
            temp.pred = temp.succ;
            temp.succ = prvPrva;
        } else if (posPrva.succ == prvVtora) {
            posVtora.succ = prvPrva;
            prvPrva.pred = posVtora;
            posPrva.succ = posleden;
            prvVtora.pred = prv;
            //lista.setFirst(prvVtora);
            //lista.setLast(posPrva);
        }

        if (prvPrva == lista.getFirst()) {
            lista.insertFirst(posVtora.element);
            lista.delete(posVtora);
        }
        if (posVtora == lista.getLast()) {
            lista.insertLast(prvPrva.element);
            lista.delete(prvPrva);
        }
        return lista;
    }
